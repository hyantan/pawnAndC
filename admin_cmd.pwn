...
CMD:unbmute( playerid, params[] ) 
{
	...
	if( Player[params[0]][uBMute] == 0 )
		return SendClient:( playerid, C_WHITE, ""gbDefault"Данный игрок не имеет блокировки OOC чата." );
...
CMD:giveitem( playerid, params[] ) 
{
	...
	if( sscanf( params, "uddd", params[0], itemid, params[2], params[3] ) ) 
		return SendClient:( playerid, C_WHITE, !""gbDefault"Введите: /giveitem [ имя/id игрока ] [ id предмета ] [ параметры ] [кол-во]");
...
CMD:createadmin( playerid, params[] )
{
	...
	if( sscanf( params, "dd", params[0], params[1] ) )
		return SendClient:( playerid, C_WHITE, !""gbDefault"Введите: /createadmin [ id игрока ] [lvl]" );
		
	if( !IsLogged( params[0] ) || params[0] == playerid )
		return SendClient:( playerid, C_WHITE, !INCORRECT_PLAYERID );
	if(params[1] > 7 || params[1] < 1) return true;
		
	mysql_format:g_string( "INSERT INTO `"DB_ADMINS"` (`aID`, `aUserID`, `aName`, `aPassword`, `aLevel`, `aStatus`, `aActive`, `aStats`, `aOnline`, `aFirstIp`) VALUES (NULL, '%d', '%s', '', '%d', '0', '1', '0|0|0', '0|0|0', '')",
 	Player[params[0]][uID],
 	Player[params[0]][uName],
	params[1]
	);
	...
	pformat:(""gbSuccess"Вы повысили ранг администратора "cBLUE"%s", Player[params[0]][uName]);
	psend:( playerid, C_WHITE );
	LoadAdminAccount(params[0]);
...
CMD:createsupport( playerid, params[] )
{
	...
	mysql_format:g_string( "INSERT INTO `"DB_SUPPORTS"` (`sID`, `sUserID`, `sName`, `sStatus`, `sActive`, `sStats`) VALUES (NULL, '%d', '%s', '0', '1', '0|0'",
 	Player[params[0]][uID],
 	Player[params[0]][uName]
	);
	mysql_tquery( mysql, g_string );
	...
	pformat:(""gbSuccess"Вы назначили "cBLUE"%s"gbSuccess" саппортом", Player[params[0]][uName]);
	psend:( playerid, C_WHITE );
	LoadSupportAccount( params[0]);
