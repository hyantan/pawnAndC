	...
	switch( dialogid ) 
	{
		...
		case d_admin + 8 :
		{
			...
			switch( listitem )
			{
				...
				case 2 :
				{
					SendClient:( playerid, C_WHITE, ""gbSuccess"Используйте команду /stats [id]" );
				}
				case 3 :
				{
					format:g_small_string( "%d", clickedplayerid );
		...
		case d_admin + 9:
		{
			...
			if (listitem == 0) {
				mysql_format:g_string( "INSERT INTO `gb_fractions` ( `f_name`, `f_leader`, 	`f_leadername_1`) VALUES ( 'New Fraction', '1|0|0', GetPlayerName(playerid))");
				mysql_tquery( mysql, g_string, "InsertFraction", "d", playerid);
			
			}
			else showPlayerDialog( playerid, d_admin + 10, DIALOG_STYLE_LIST, " ", ""cWHITE"\
					Информация\n\
		...
		case d_admin + 32:
		{
			...
			for( new i; i < MAX_CRIMINAL; i++ )
			{
				if( !CrimeFraction[i][c_id] )
				{
					...
					mysql_format:g_string( "INSERT INTO `"DB_CRIME"` ( `c_name`, `c_leader` ) VALUES ( 'Crime Fraction', '0|0|0' )");
		...
		case d_makeleader + 1 : 
		{
			...
			{
				SendClient:( playerid, C_WHITE, ""gbError"Данный игрок не является лидером");
				format:g_small_string( "%d", fid + 1 );
				return cmd_unleader( playerid, g_small_string );
			}
